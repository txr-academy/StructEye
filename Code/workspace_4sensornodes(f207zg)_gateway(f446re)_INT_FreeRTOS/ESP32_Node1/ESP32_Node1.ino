/*
 * StructEye - ESP32 Sensor Node 1 (FreeRTOS Multi-Task)
 * Node ID: 0x01
...
void vThresholdDetectionTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (!rxData.sensor_ok)   status_flags |= (1 << 2);
    if (seq_num == 0)        status_flags |= (1 << 3);
    if ((status_flags & 0x03) != 0) alert_level = 2;
    ...
    if (xQueueSend(xTxQueue, &txPacket, pdMS_TO_TICKS(100)) == pdPASS) {
      lastTxTick = now;
      Serial.printf("[PROC] Seq=%u Vib=%.2f Tilt=%.2f Alert=%u\n",
