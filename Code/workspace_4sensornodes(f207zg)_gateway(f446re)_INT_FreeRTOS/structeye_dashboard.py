...
try:
    ...
    print("Run:  py -m pip install paho-mqtt")
    sys.exit(1)
...
ALERT_COOLDOWN = 60 # seconds between alerts per node
last_alert_time = {}

# Initialize MQTT Client
MQTT_BROKER = "broker.emqx.io"
MQTT_PORT = 1883
# Generate a random 8-character ID for this session to prevent topic collision
SESSION_ID = "structeye_" + "".join(random.choices(string.ascii_lowercase + string.digits, k=8))
print(f"*** MOTT CLOUD ENABLED! Your unique Session ID is: {SESSION_ID} ***")
...
try:
    ...
    print(f"Warning: Could not connect to MQTT broker {MQTT_BROKER}: {e}")
# Initialize CSV Log File
LOG_FILE = "structeye_data_log.csv"
if not os.path.exists(LOG_FILE):
    with open(LOG_FILE, mode='w', newline='') as f:
        csv.writer(f).writerow(['Timestamp', 'Node', 'Seq', 'Accel_X', 'Accel_Y', 'Accel_Z', 'Gyro_X', 'Gyro_Y', 'Gyro_Z', 'Vib_m_s2', 'Tilt_deg', 'Status', 'Alert', 'Total_RX', 'Missed'])
...
def commit():
    ...
    with lock:
        ...
        # --- NEW: Save to CSV Log File ---
        try:
            a_name = ["OK", "WARN", "CRITICAL"][n.alert] if 0 <= n.alert <= 2 else "UNKNOWN"
            alert_text = f"{n.alert} ({a_name})"
            with open(LOG_FILE, mode='a', newline='') as f:
                csv.writer(f).writerow([
                    n.last_seen.strftime("%Y-%m-%d %H:%M:%S"),
                    n.nid, n.seq, n.ax, n.ay, n.az, n.gx, n.gy, n.gz,
                    round(n.vib, 2), round(n.tilt, 2), f"0x{n.status:02X}", alert_text, n.packet_count, n.missed
                ])
        except Exception:
            pass
        ...
        try:
            payload = {
                ...
                "node_id": n.nid,
                "sequence": n.seq,
                "accel": {"x": n.ax, "y": n.ay, "z": n.az},
                "gyro": {"x": n.gx, "y": n.gy, "z": n.gz},
                "vibration_m_s2": round(n.vib, 2),
                "tilt_deg": round(n.tilt, 2),
                "status_flags": n.status,
                "alert_level": n.alert,
                "alert_name": a_name,
                "packet_count": n.packet_count,
                "missed_packets": n.missed
            }
            mqtt_client.publish(f"{SESSION_ID}/telemetry/node_{n.nid}", json.dumps(payload))
        except Exception:
            pass
        # --- NEW: Telegram Push Alerts ---
        if n.alert > 0 and TELEGRAM_BOT_TOKEN != "YOUR_BOT_TOKEN_HERE":
            now = time.time()
            if n.nid not in last_alert_time or (now - last_alert_time[n.nid]) > ALERT_COOLDOWN:
                last_alert_time[n.nid] = now
                msg = f"{a_name} ALERT 🚨\n\nNode {n.nid} detected high structural activity!\nVibration: {n.vib:.2f} m/s²\nTilt: {n.tilt:.2f}°"
                url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
                try:
                    # Fire-and-forget thread to avoid blocking serial reads
                    threading.Thread(target=requests.post, args=(url,), kwargs={"json": {"chat_id": TELEGRAM_CHAT_ID, "text": msg}, "timeout": 3}, daemon=True).start()
                except Exception:
                    pass
...
def header():
    ...
    t.append(f"  🔑 SESSION: {SESSION_ID}  ",style="bold bright_magenta on black")
